...
class BiasedBeliefNetwork(
  ...
  /** Calculate the coherence-value from all constraints and biased beliefs with
   * the given truth-value assignment.
   *
   * @param assignment
   *  A truth-value assignment over vertices
   *  @return
...
object RandomBiasedBeliefNetwork {
  ...
  def random(
    ...
    val biasBeliefs: List[Node[String]] = network.vertices.take(nrBiasBeliefs).toList // Determine which vertices become biased beliefs
    val nrTrueBias: Int = (ratioBiasTrueFalse*nrBiasBeliefs).intValue // Determine number of '''true''' biases
    val biasTruthValues: List[Boolean] = Random.shuffle(List.fill(nrTrueBias)(true) ++ List.fill(nrBiasBeliefs - nrTrueBias)(false)) // Generate truth-values
    val biasAssignment: Map[Node[String], Boolean] = (biasBeliefs zip biasTruthValues).toMap // Assign truth values to biased beliefs
    val biasWeights: Map[Node[String], Double] = biasBeliefs.map((v: Node[String]) => v -> Random.nextDouble()).toMap // Generate bias weights
  ...
  def random(
    ...
    val biasBeliefs: List[Node[String]] = Random.shuffle(vertices.toList).take(nrBiasBeliefs)// Determine which vertices are biased beliefs

    val biasTruthValues: List[Boolean] = Random.shuffle(List.fill(nrBiasBeliefs - nrFalseBiases)(true) ++ List.fill(nrFalseBiases)(false)) // Generate truth-values
    val biasAssignment: Map[Node[String], Boolean] = (biasBeliefs zip biasTruthValues).toMap // Assign truth values to biased beliefs
    val biasWeights: Map[Node[String], Double] = biasBeliefs.map((v: Node[String]) => v -> Random.nextDouble()).toMap // Generate bias weights
