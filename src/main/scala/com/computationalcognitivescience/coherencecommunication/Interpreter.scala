...
class Interpreter(
   ...
   * @return
   * A repair request (truth-value assignment over nodes) if the new coherence is lower than the previous one.
   */
  ...
  /** Based on (van Arkel, 2021, p. 24)
   * Generate a repair request (truth-value assignment)
   *
   * @param beliefNetwork
   * A network of beliefs
   * @param inferredBeliefs
      ...
      powerset(vRequest) // Take the powerset over beliefs that may be requested
        ...
        .map((vRequest: Set[Node[String]]) =>
          vRequest // Take the set of requested beliefs
            .map((node: Node[String]) => (node, !inferredBeliefs(node))) // Flip the truth-value assignment
            .toMap
        )
    }
